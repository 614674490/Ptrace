...
int command_step(struct dbug_struct *dbug, void *arg)
{
    ...
    ptrace(PTRACE_SINGLESTEP, dbug->pid, NULL, NULL);
    ...
    //让子进程在调用系统调用时暂停
    if (ptrace(PTRACE_SYSCALL, dbug->pid, NULL, NULL) < 0)
    {
